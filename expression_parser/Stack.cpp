...
Stack& Stack::operator=( const Stack& rhs ){
	char me[] = "Stack=";
 	DBG(me, "using assignment op", DEBUG_V);
	if( &rhs != this ){
		Node* cursor = head;
		Node* prev = NULL;
		while( cursor != NULL ){
...
void Stack::push( const list_item& new_data ) {
	...
	head = new_node;
	num_elements++;
	DBG(me, "Pushed a node to stack", DEBUG_V );
}
